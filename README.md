...
A Makefile is provided in `plot` subdirectory to do the plotting.
The training log needs to be saved in `data/MODEL_NAME.log`.

As an example, use the training data provided in 
[ocrd-testset.zip](./ocrd-testset.zip) to do training and generate the plots.
Plotting can be done while training is running to depict the training status till then.
Generated plot and TSV files are saved in `data/plot` directory.
```
mkdir -p data
unzip ocrd-testset.zip -d data/ocrd-ground-truth
nohup make training MODEL_NAME=ocrd START_MODEL=frk TESSDATA=$HOME/tessdata_best MAX_ITERATIONS=10000 > data/ocrd.log &
```
Allow training to run for some time, till a few checkpoints with CER less than 10% are generated, 
before running evaluation with validation list and plotting the results. 
```
cd plot
make MODEL_NAME=ocrd traineddata
